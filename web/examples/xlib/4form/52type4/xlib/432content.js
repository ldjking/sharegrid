...
(function(x){	/*menu效果*/
	...
	var e_checkbox_click=function(evt){
		var status=this.checked;
		var tbody=this.parentNode.parentNode.parentNode.parentNode;
		var checkboxs=$(".itemcheckbox",tbody);
	...
	var genChildTableContent2=function(cfg,line,rowNum,tDef,childData){
		var lineTable=$table(rowNum,tDef.columns.length);/*默认放4行内容*/
		/*要把不同的row划分出来*/
		/*子表的数据要重新计算*/
		lineTable.className="childs";
		...
		for(var j=0;j<tDef.columns.length;j++){
			...
			/*if(j==0){
				...
				cell.style.width=cellDef.width-20+"px";
				cellTd.className="lbl";
				var checkbox=$checkbox("","maincheckbox");
				cell.appendChild(checkbox);
				cellTd.appendChild(cell);
				x.bind(checkbox,"click",e_checkbox_click);
				for(var z=1;z<rowNum;z++){
					var obj=childData[z-1];
					lineTable.rows[z].data=obj;
					var cellTd2=lineTable.rows[z].cells[j];
					var cell2=$checkbox("","itemcheckbox");
					cellTd2.appendChild(cell2);
					if(obj[tDef.pkAttr]) cell2.checked=true;
				}
			}
			else{*/
				var cellDef=tDef.columns[j];
				var cellTd=lineTable.rows[0].cells[j];/*先生成表头 */
				if(cellDef.type=="hide") cellTd.style.display="none";
				var cell=$e("div");
				cell.style.width=cellDef.width-20+"px";
				cellTd.className="lbl";
				cell.innerHTML=cellDef.name;
				...
				for(var z=1;z<rowNum;z++){/*生成剩余的行*/
					...
					////out("obj",obj);
					var cellTd2=lineTable.rows[z].cells[j];/*再生成3行空白行供输入 */
					if(cellDef.type=="hide") cellTd2.style.display="none";
					var cell2=$text("text");
					cell2.cellDef=cellDef;
					cellTd2.appendChild(cell2);
					x.bind(cell2,"mousedown",e_text_mousedown);
	...
	var genContent=function(form){
		...
		for(var i=0;i<formTable.rows.length;i++){
			...
			if(tDef.type==0){/*隐藏元素*/
			...
			else if(tDef.type==3||tDef.type==4){/*代表是子表*/
				...
				if(cfg.formType=="add"){
					...
					if(tDef.type==3)genChildTableContent(cfg,line,rowNum,tDef,childData);
					else if(tDef.type==4){
						 if(data[tDef.dependeAttr]!=null) genChildTableContent2(cfg,line,rowNum,tDef,childData);
					}
				}
	...
	x.genFormContent=function(form){
		...
		/*step1 生成表单标题*/
		var data=cfg.data;
		if(cfg.dataMethod!=null&&cfg.notReadDate==null){
			/*要取这个数据*/
