(function(x){
	...
	var e_nav_click=function(evt){/*打开导航栏*/
		//out("nav click");	
		var target=x.getTarget(evt);
		var repeater=$1(".repeater",target,0);
		var cfg=repeater.repeaterCfg;
		var container=x.$("#"+cfg.id+"_container");

		if(cfg.navState){
			x.hideRepeaterNav(repeater);
		}
		else{
			x.showRepeaterNav(repeater);;/*进入导航状态*/
	...
	var e_delete_click=function(evt){
		...
		if(cfg.activeRow!=null){
			...
			x.$$(cfg.rmMethod,param,false,function(result){
					if(result.flag)	x.genRepeaterContent(repeater);
	...
	var e_addItem_click=function(evt){/*新增一项*/
		var target=x.getTarget(evt);
		var repeater=$1(".repeater",target,0);
		x.addRepeaterRow(repeater);
	}
	var e_rmItem_click=function(evt){/*删除一项*/
		var target=x.getTarget(evt);
		var repeater=$1(".repeater",target,0);
		x.rmRepeaterRow(repeater);
	}
	var e_lock_click=function(evt){/**/
		var target=x.getTarget(evt);
		var repeater=$1(".repeater",target,0);
		var id="#"+repeater.repeaterCfg.id+"_";
		var cmd_addItem	=x.$(id+"cmdAddItem");
		var cmd_rmItem	=x.$(id+"cmdRmItem");
		var cmd_commit	=x.$(id+"cmdCommit");
		var flag=repeater.repeaterCfg.editMode;
		if(flag){
			x.closeRepeaterEdit(repeater);
			target.className="lock";
			x.cssAdd([cmd_addItem,cmd_rmItem,cmd_commit],"gray");
		}
		else{
			x.makeRepeaterEdit(repeater);
			target.className="unlock";
			x.cssRm([cmd_addItem,cmd_rmItem,cmd_commit],"gray");
		}
	}
	var e_commit_click=function(evt){/*更新repeater*/
		var repeater=$1(".repeater",x.getTarget(evt),0);
		saveRepeaterChange(repeater);
	}
	
	var e_filter_click=function(evt){/*数据筛选*/
		var repeater=$1(".repeater",x.getTarget(evt),0);
		if(repeater.repeaterCfg.filterMode) x.closeRepeaterFilter(repeater);/*已经是筛选模式*/
		else	x.openRepeaterFilter(repeater);
	}
	var e_refresh_click=function(evt){/*刷新*/
		...
		cfg.filter={};
		x.closeRepeaterFilter(repeater);
		x.genRepeaterContent(repeater);
		/*逐项清楚filter里面的内容 */
	}
	
	var checkCmd=function(repeater){
		var cfg=repeater.repeaterCfg;
		var id=cfg.id;
		var cmd=$("#"+id+"_cmd");
	...
	x.genRepeaterCmd=function(repeater){
		/*为一个网格对象生成操作按钮*/
		var cfg=repeater.repeaterCfg;
