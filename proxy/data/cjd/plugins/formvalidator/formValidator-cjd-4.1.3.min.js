(function($) {
	$.formValidator = {
		initConfig : function(controlOptions) {
				...
				$("#" + settings.formID).submit(
						function() {
							if (settings.ajaxForm != null) {
								$.formValidator.ajaxForm(
										settings.validatorGroup,
										settings.ajaxForm);
								return false
							} else {
			...
			if ($.browser.msie) {
				...
				css.rel = "stylesheet";
				css.type = "text/css";
				css.href = themedir + "themes/" + settings.theme
						+ "/style/style.css";
				document.getElementsByTagName("head")[0].appendChild(css)
			} else {
				var style = document.createElement('style');
				style.setAttribute("type", "text/css");
				var styCss = "@import url('" + themedir + "themes/"
						+ settings.theme + "/style/style.css');";
				if (style.styleSheet) {
			...
			if (initConfig.mode == "SingleTip") {
				...
				var html = showclass == "onShow" ? onShowHtml
						: (showclass == "onFocus" ? onFocusHtml
								: (showclass == "onCorrect" ? onCorrectHtml
										: onErrorHtml));
				if (html.length = 0) {
					tip.hide()
				} else {
					if (elem.validatorPasswordIndex > 0
							&& showclass == "onCorrect") {
						...
						showmsg = passwordStrengthText[level <= 0 ? 0
								: level - 1]
					}
					html = html.replace(/\$class\$/g, showclass).replace(
							/\$data\$/g, showmsg);
					if (showclass != "") {
		...
		resetTipState : function(validatorGroup) {
			...
			$.each(initConfig.validObjects, function() {
				...
				$.formValidator.setTipState(this, passed ? "onCorrect"
						: "onShow", passed ? $.formValidator.getStatusText(
						this, setting.onCorrect) : setting.onShow)
			})
		},
		setFailState : function(tipID, showmsg) {
			$.formValidator.setTipState($("#" + tipID).get(0), "onError",
					showmsg)
		},
			...
			if (!isValid) {
				...
				if (setting.validateType == "ajaxValidator") {
					if (setting.lastValid == "") {
						...
						showmsg = $.formValidator.getStatusText(elem,
								setting.onError)
					}
				} else {
					showmsg = (returnObj.errormsg == "" ? $.formValidator
							.getStatusText(elem, setting.onError)
							: returnObj.errormsg)
				}
				...
				showmsg = $.formValidator.isEmpty(id) ? setting.onEmpty
						: $.formValidator
								.getStatusText(elem, setting.onCorrect);
		...
		showAjaxMessage : function(e) {
			...
			e.setting = setting;
			if (validValueOld != validvalue || validValueOld == validvalue
					&& elem.onceValided == undefined) {
				$.formValidator.ajaxValid(e)
			} else {
				if (setting.isValid != undefined && !setting.isValid) {
					elem.lastshowclass = "onError";
					elem.lastshowmsg = $.formValidator.getStatusText(elem,
							setting.onError)
				}
		...
		pageIsValid : function(validatorGroup) {
			...
			$.each(initConfig.validObjects, function() {
				...
				if (this.settings[0].bind
						&& this.validatorAjaxIndex != undefined
							...
							function() {
								...
								if (this.settings[0].bind) {
									...
									if (name_list.indexOf("^" + name + "^") == -1) {
										onceValided = this.onceValided == undefined ? false
												: this.onceValided;
										if (name) {
										...
										if (returnObj) {
											if (!returnObj.isValid) {
												isValid = false;
												errorMessage = returnObj.errormsg == "" ? returnObj.setting.onError
														: returnObj.errormsg;
			...
			if (isValid) {
				...
				initConfig.onError(firstErrorMessage, $("#" + thefirstid)
						.get(0), errorlist);
		...
		ajaxValid : function(e) {
			...
			parm = "clientid="
					+ id
					+ "&"
					+ (setting.randNumberName ? setting.randNumberName
							+ "="
							+ ((new Date().getTime()) + Math.round(Math
									.random() * 10000)) : "")
					+ (parm.length > 0 ? "&" + parm : "");
			if (setting.type == "POST") {
				ls_data = ls_data + (ls_data != "" ? ("&" + parm) : parm)
			...
			$.ajax({
				...
				success : function(data, textStatus, jqXHR) {
					...
					if ((typeof lb_ret) == "string") {
						...
						ls_status = "onError";
						ls_msg = $.formValidator.getStatusText(elem,
								setting.onError)
					}
					setting.isValid = lb_isValid;
					$.formValidator.setTipState(elem, ls_status, ls_msg);
					if (e.initConfig.status == "sumbitingWithAjax"
							&& e.initConfig.ajaxCountSubmit == 0) {
		...
		regexValid : function(returnObj) {
			...
			if (elem.settings[0].empty && elem.value == "") {
				...
				$.each(regexArray, function() {
					...
					isValid = (new RegExp(r, setting.param))
							.test($(elem).val());
		...
		passwordValid : function(elem) {
			...
			;
			if (setting.compareID != ""
					&& $("#" + setting.compareID).val() == pwd) {
				return -3
			}
			;
			var sum1 = [ 0, 0, 0, 0 ];
	...
	$.fn.formValidator = function(cs) {
		...
		return this
				.each(function(e) {
					...
					var tip = setting_temp.tipID ? setting_temp.tipID : id
							+ "Tip";
					if (initConfig.mode == "AutoTip"
							|| initConfig.mode == "FixTip") {
						var tipDiv = $("#" + tip);
						if (initConfig.mode == "AutoTip" && tipDiv.length == 0) {
							var relativeID = setting_temp.relativeID ? setting_temp.relativeID
									: id;
							var offset = $("#" + relativeID).offset();
							setting_temp.tipCss.top = offset.top
									+ setting_temp.tipCss.top;
							setting_temp.tipCss.left = $("#" + relativeID)
									.width()
									+ offset.left + setting_temp.tipCss.left;
							tipDiv = $(
						...
						tipDiv.css("margin", "0 0 0 80px")
								.css("padding", "0px").css("background",
										"transparent")
					} else if (initConfig.mode == "SingleTip") {
						jqobj.showTooltips()
					}
					setting.tipID = tip;
					setting.pwdTipID = setting_temp.pwdTipID ? setting_temp.pwdTipID
							: setting.tipID;
					setting.fixTipID = setting_temp.fixTipID ? setting_temp.fixTipID
							: id + "FixTip";
					...
					var isInputControl = stype == "password" || stype == "text"
							|| stype == "textarea";
					...
					if (fixTip.length == 1 && onMouseOutFixTextHtml != ""
							&& onMouseOnFixTextHtml != "" && showFixText != "") {
						jqobj.hover(function() {
							fixTip.html(onMouseOnFixTextHtml.replace(
									/\$data\$/g, showFixText))
						}, function() {
							fixTip.html(onMouseOutFixTextHtml.replace(
									/\$data\$/g, showFixText))
						});
						fixTip.css("padding", "0px 0px 0px 0px").css("margin",
								"0px 0px 0px 0px").html(
								onMouseOutFixTextHtml.replace(/\$data\$/g,
										setting.onShowFixText))
					}
					...
					if (srcTag == "input" || srcTag == "textarea") {
						if (isInputControl) {
							if (showText != "" && jqobj.val() == "") {
								...
								initConfig.showTextObjects = showObjs
										+ (showObjs != "" ? ",#" : "#") + id;
								jqobj.val(showText);
								jqobj.css("color",
										setting.onShowTextColor.mouseOutColor)
							}
						}
						jqobj
								.focus(function() {
									if (isInputControl) {
										...
										if (showText == val) {
											...
											jqobj
													.css(
															"color",
															setting.onShowTextColor.mouseOnColor)
										}
									}
									;
									if (initConfig.mode != "AlertTip") {
										var tipjq = $("#" + tip);
										this.lastshowclass = tipjq
												.attr("class");
										...
										function() {
											if (this.validatorPasswordIndex > 0) {
												try {
													...
													$("#" + setting.pwdTipID)
															.html(
																	passwordStrengthStatusHtml[level])
												} catch (e) {
						...
						jqobj.bind({
							...
							blur : function() {
								if (this.validValueOld == undefined
	...
	$.fn.compareValidator = function(controlOptions) {
		var settings = {};
		$
				.extend(true, settings, compareValidator_setting,
						controlOptions || {});
		return this.each(function() {
			$.formValidator.appendValid(this.id, settings)
		})
	...
	$.fn.ajaxValidator = function(controlOptions) {
		...
		return this.each(function() {
			...
			this.validatorAjaxIndex = $.formValidator.appendValid(this.id,
					settings)
		})
	};
	$.fn.passwordValidator = function(controlOptions) {
		...
		$.extend(true, settings, passwordValidator_setting, controlOptions
				|| {});
		return this.each(function() {
			this.validatorPasswordIndex = $.formValidator.appendValid(this.id,
					settings)
		})
	};
				...
				.each(function() {
					...
					for ( var i = 1; i < settings.length; i++) {
						...
						if (!($("body").data(settings[0].validatorGroup).mode == "AlertTip")) {
							this.lastshowclass = onShow ? "onShow"
									: "onCorrect";
							this.lastshowmsg = onShow ? settings[0].onShow
									: settings[0].onCorrect;
							$.formValidator.setTipState(this,
									this.lastshowclass, this.lastshowmsg)
						}
					}
				})
	};
	...
	$.fn.showTooltips = function() {
		if ($("body [id=fvtt]").length == 0) {
			...
			$("body").append(fvtt);
			fvtt
					.before("<iframe index=0 src='about:blank' class='fv_iframe' scrolling='no' frameborder='0'></iframe>")
		}
		return this
				.each(function() {
					jqobj = $(this);
