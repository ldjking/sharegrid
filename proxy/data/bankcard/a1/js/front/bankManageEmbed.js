...
function getTenpayBarchData(){
	...
	$.ajax({
	 	type:"post",
	 	url:"data/bangka/step1/tenpayBarchData.do",
	 	data:"bankcode="+bankCode+"&selfid="+d.cId,
	 	success:function(data){
			console.log("getTenpayBarchData",data);
...
(function(){
	...
	jQuery(".province span").click(function(){
		...
		$.ajax({
		 	type:"post",
		 	url:"data/bangka/step1/getareadata.do",
	...
	function checkParam(){
		if(checkCardNo()){
			...
			$.ajax({
			 	type:"post",
			 	url:"data/bangka/step1/checkcarddcflag.do",
			 	...
			 	success:function(data){
					if(typeof(data)=="string"){
			 		...
			 		if(data.key == 1){
			 			var param = {};
						param["paramMap.dealpwd"] = RSAUtils.pwdEncode(jQuery("#txpass").val());
			 			if(jQuery("#txpass").length>0){
			 				$.ajax({
			 				 	type:"post",
			 				 	url:"data/bangka/step1/getUserTxPwdEqLogPwd.do",
			 				 	data:param,
			 				 	success:function(data){
									if(typeof(data)=="string"){
										try{
	...
	function getParam(){
		var param = {};
			...
			//提现密码
			param["paramMap.dealPwd"] = RSAUtils.pwdEncode($("#txpass").val()||"");
			//默认提现卡
	...
	jQuery("#submitToCheckBank").bind("click",function(){
		...
		$.ajax({
			type:"POST",
			url:"data/bangka/step1/tenpaybankInsert.do",
			dataType:"json",
			asyc:true,
			data:param,
			success:function(data){
	...
	jQuery("#submitButtonV").bind("click",function(){
		...
		if(checkMoney(value)){
			...
			$.ajax({
				type:"POST",
				url:"data/bangka/step1/tenpayBankMoneyVali.do",
				dataType:"json",
				asyc:true,
				data:param,
				success:function(data){
					...
					if(data.key == 1){
						...
						jQuery("#closeFaultDialog").addClass("disabledButton2").removeAttr("href");
						timeLeft(5);
	...
	window.timeLeft=function(t){
		if(t){
			$("#leftTime").text("("+(--t)+")");
			setTimeout(function(){timeLeft(t)},1000);
		}else{
			$("#leftTime").html("");
			$("#closeFaultDialog").attr("href","javascript:closeDialog()").removeClass("disabledButton2");
		}
	}
	...
	function subSuccess(data){
		...
		$.post("data/bangka/step1/tenpaybankSubSuccess.do",param,initCallBack);
	};
	function subMoneySuccess(data){
		var param = {};
		param["type"] = "money";
		$.post("data/bangka/step1/tenpaybankSubSuccess2.do",param,initCallBack);
	};
